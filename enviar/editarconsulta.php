...
<?php include 'sidebar.php' ?>
            ...
            <div class="main__container">
                ...
                ?>
                    <form class="form__busqueda-propiedad form" id="editConsultaForm" autocomplete="off" name="form" method="POST" action="backend/editar.php?page=consulta&consulta=<?php echo $editarId;?>">
                        ...
                        <div class="form__bloque">
                            <div class="form__bloque__content content">
                                <input type="text" placeholder="Ingrese nombre de zona" class="form__text content__text" name="buscadorZonas" id="buscadorZonas" autocomplete="off"> 
                                <div class="zonasSelected" id="zonasSeleccionadas" <?php if(!empty($editarBuscarZona) OR $editarBuscarZona != '' OR $editarBuscarZona){echo 'style="display:flex"';}?>>                              
                                </div>


                                <input type="hidden" class="form__text content__text" name="buscarZona[]" id="buscarZona" value="<?php echo $editarBuscarZona;?>">
                                                        
                                <ul class="content_ul" id="listaZonas"></ul>                
                            </div>  
                        </div>                                    
                        <div class="main__decoration"></div>
    ...
    <?php
        if(!empty($editarBuscarZona) OR $editarBuscarZona != '' OR $editarBuscarZona){
            $editarBuscarZonaArr = explode(",", $editarBuscarZona);
            foreach ($editarBuscarZonaArr as $zona) {
            $sentencia = $connect->prepare("SELECT * FROM `wp_zonas` WHERE id=? AND HABILITADA = 1 AND nombre !='' ") or die('query failed');
            $sentencia->execute([$zona]);
            $zonas = $sentencia->fetchAll();
            foreach($zonas as $zona){
                $idZona = $zona['id'];
                $zonaNombre = $zona['nombre'];
                echo '<script>agregarZonaSeleccionada('.$idZona.', "'.$zonaNombre.'");</script>';
            }
            }
        }
    ?>
