<?php
// Agregar propiedad //
function agregarPropiedad($connect,$connect2): void{   
    ...
    if($_POST['precioalquiler'] == ''){$_POST['precioalquiler']= 0;};
    ...
    if (!empty($_FILES['galeriafotos']['name'][0])){
        for ($i = 0; $i < count($_FILES['galeriafotos']['name']); $i++) {
            ...
            $_FILES['galeriafotos']['name'][$i] = $i.'__galeria_fotos__'.date('Y_m_d_H_i_s') . '__' . $_FILES['galeriafotos']['name'][$i];
        }
    ...
    if($query){
        ...
        if($query){
            ...
            if($captadoPor != $createdBy){
                $mensaje = '<a href="propiedadesinfo.php?ref='.$ref.'"><i class="fa-solid fa-building"></i>'.$nombreNotificaciones.' '.$apellidoNotificaciones.' te ha asignado una propiedad</a>';
                $query = $connect-> prepare ("INSERT INTO wp_notificaciones (mensaje, user_id, seen, fecha) VALUES (?, ?, ?, ?)");
                $query->execute([$mensaje, $captadoPor, 0, $created]);
            }
            if (!empty($_FILES['fotoportada']['name'])){
                // Conexion ftp  
...
function agregarConsulta($connect) : void{
    ...
    if($query){
        ...
        if($asignadoA != $createdBy){
            ...
            $mensaje = '<a href="consultasinfo.php?consulta='.$idConsulta.'"><i class="fa-solid fa-circle-question"></i>'.$nombreNotificaciones.' '.$apellidoNotificaciones.' te ha asignado una consulta</a>';
            $query = $connect-> prepare ("INSERT INTO wp_notificaciones (mensaje, user_id, seen, fecha) VALUES (?, ?, ?, ?)");
...
function agregarUsuario($connect,$connect2) : void{
    ...
    $query = $connect-> prepare ("INSERT INTO usuarios (nombre, apellido, nickname, pass, rol, celular, email, sobre_mi, foto, habilitado, sucursal_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
// Editar propiedad //
function editarPropiedad($connect,$connect2): void{   
    if(!isset($_POST['operacion'])){$_POST['operacion']= '';};
    if(!isset($_POST['tipo'])){$_POST['tipo']= '';};
    if(!isset($_POST['codigopostal'])){$_POST['codigopostal']= '';};
    if(!isset($_POST['tipocalle'])){$_POST['tipocalle']= '';};
    ...
    if($_POST['contacto_id'] == ''){$_POST['contacto_id']= 0;};
        ...
        if($NEWprecioAlquiler != $editarPrecioAlquiler){
            $update .= ", modified_by = '".$NEWprecioAlquiler."'";
        }
...
// Eliminar propiedad //
function eliminarPropiedad($connect,$connect2) : void{
    $id = $_GET['id'];
